...
Validated with what is checkable *without* fEGarch fixtures: every density integrates to 1 with mean
0 and variance 1; `cdf`/`ppf` round-trip; the seeded sampler's moments match analytically; each skew
**reduces exactly to its symmetric base at ξ = 1** (a clean known-truth anchor); and QMLE **recovers
planted GARCH(1,1) parameters** with a log-likelihood that matches a hand computation. Two
parameterizations carry `# RECONCILE` flags — the exact **ALD** form and the exact **Fernández–Steel
normalization constants** — implemented as documented defaults behind a stable API, to be locked once
the fixtures exist. **The fEGarch-fixture agreement checks are deferred, as clearly-named *skipped*
test stubs**, so nothing silently passes as "matches fEGarch" before it has been checked. **Next:
generate and commit the R output fixtures, then the short-memory models (Phase 1).**
