...
(i) **the R output-fixture pipeline is now STOOD UP ✓** (step 24 — `tests/fixtures/fegarch/`, fEGarch
1.0.6 / R 4.6.1, generated by running fEGarch's *public* functions only, R never in CI); the
remaining piece is to **wire the committed fixtures into the Python tests** (fill the clearly-named
*skipped* stubs `test_*_matches_fegarch_fixture`) — nothing yet claims fEGarch agreement;
(ii) resolve the **two `# RECONCILE` flags** against paper + fixture — the exact **ALD**
(average-Laplace) density and the exact **Fernández–Steel normalization constants**; the fixtures
already **inform** the ALD flag: fEGarch's `ald` is parameterized by a `P` (default 8) and at P=8 is
near-normal (raw kurtosis ≈ 3.3), so the current standardized-Laplace default (kurtosis 6) is **wrong**
— the exact form needs the paper; (iii) **source the exact specification papers** for each later
model's parameterization. **Three load-bearing foundations everything reuses:** the conditional-distribution
