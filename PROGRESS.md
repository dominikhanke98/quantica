...
curve could not. A **fifth pillar — time series / econometrics** is now **complete end-to-end** (step 20:
...
smoother / EM, recovering planted regimes and detecting the 2008 crisis; step 22: multivariate —
VECM generalising pairwise cointegration and DCC-GARCH producing a time-varying covariance that
feeds the portfolio/risk pillars, the cross-pillar coherence payoff). Next: optional depth only —
see "Next".
...
Time-series / econometrics roadmap (**Pillar V — forecast-evaluation-first, COMPLETE**): **GARCH
...
PR #13 merged `b1d2348`) → **multivariate ✓** (VECM via Johansen reduced-rank generalising pairwise
cointegration; DCC-GARCH time-varying covariance feeding the portfolio/risk pillars — PR open). The
GARCH-step headline is *validate-the-validator* (the DM test is correctly sized only with HAC); the
regime-switching headline is *known-truth state recovery* (filter+EM recover planted regimes,
anchored to statsmodels' `MarkovRegression` at ~1e-15); the multivariate headline is the
**cross-pillar tie-back** (DCC's conditional covariance is exactly what the portfolio/risk pillars
consume — one pillar's output is another's validated input). **Pillar V closes the econometrics arc.**
...
**`fEGarch` clean-room port (sub-project within Pillar V — Phase 0 COMPLETE ✓, PR #15 merged
`89ff4e2`).** A Python reimplementation of the `fEGarch` R package's model family (Schulz, Feng et
al., Paderborn — v1.0.6, GPL-3): a broad EGARCH-type family with **long-memory /
fractionally-integrated** members (FIEGARCH / FILog-GARCH / FIMLog-GARCH) that no mainstream Python
library offers. **Phase 0 done + merged: `quantica/timeseries/fegarch/` — the conditional-distribution
layer + the QMLE engine, both validated against committed `fEGarch` fixtures** (see step below).
Full roadmap in [`docs/fegarch-port-roadmap.md`](docs/fegarch-port-roadmap.md); standing rules
...
specs → GPL-3 copyleft does not attach). **Phase 0 status — DONE:** (i) the R output-fixture pipeline
is stood up ✓ (`tests/fixtures/fegarch/`, fEGarch 1.0.6 / R 4.6.1, run from fEGarch's *public*
functions only, R never in CI) **and wired into the Python tests** ✓ — the layer now matches fEGarch;
(ii) **both `# RECONCILE` flags RESOLVED** ✓ against paper + fixture — the **ALD** is the **scaled
average-Laplace (Sargan)** density (App. C.1 Eqs. 31–33, 37; raw kurtosis `3 + 3/(P+1)`, `P` a fixed
integer construction param profiled over a grid, so `param_names = ()`), replacing the wrong
standardized-Laplace placeholder; the **Fernández–Steel** normalization is standardized per App. C.1
Eqs. 38–41 (equal to the Lambert–Laurent form) with `skew = ξ` **directly** (`skew<1` left-skew,
`>1` right-skew), confirmed against the fixtures; derivations recorded in
[`docs/fegarch-spec-notes.md`](docs/fegarch-spec-notes.md). **Known validation margin (recorded):**
the distribution fixtures are **1e6-draw Monte-Carlo estimates** from fEGarch's public *samplers*
(fEGarch exposes no public density/CDF/quantile functions under the clean-room rule), so the
density/quantile match is within **MC tolerance ~0.02** (worst realized deviation 0.0153, ≈1.3×
headroom; every asserted deviation is ≤2.8 sampling-SE — genuine noise, not a masked disagreement).
The lever, if the margin ever tightens, is **higher-N regeneration** (1e8 draws → noise ~1.5e-3,
atol ~3e-3). **Three load-bearing foundations everything reuses:** the conditional-distribution
...
**Phase 6 multi-step `predict` — COMPLETE + MERGED** (PR #25 → `main` `5b3bf50`, CI green on
py3.11/3.12), **completing roadmap Phase 6**. `predict(fit, returns, n_ahead)` (`quantica/…/forecast.py`)
...
bit-for-bit; a test locks the EGARCH biased limit. μ̂ constant (dual-mean multi-step mean deferred);
unsupported families raise `NotImplementedError`. Spec: `docs/fegarch-spec-notes.md` §29.

**ROADMAP PHASE 6 IS COMPLETE — and the fEGarch clean-room port is complete through the entire required
roadmap.** All fixture-validated: modelling (short-memory, EGF, fractional/long-memory), the full
eight-distribution breadth, the dual mean (ARMA/FARIMA), forecasting (`predict` + `predict_roll`),
VaR/ES + the risk-pillar backtest tie-back (Kupiec/Christoffersen/Basel/Acerbi-Székely), and the
residual diagnostics (weighted Ljung-Box / sign-bias / GoF). **What remains is explicitly OPTIONAL:**
(1) **Phase 7 — semiparametric local-polynomial scale** (an add-on, not required); (2) **deferred
forecasting breadth** — multi-step `predict` for the other distributions / the dual-mean (non-constant
...
- **Step 17 — Rates / fixed-income pillar opened: yield-curve construction.** New top-level
  ...
  **the headline — interpolation changes forwards** (same inputs, all repriced, forwards diverge
  ...
  Delivered on branch `feat/rates-curve` as **PR #9 — open, CI-green (py3.11, py3.12, benchmark,
  docs all pass), awaiting review** (not yet merged; the merge is left to the author). **CI
...
- **Step 21 — Pillar V step 2: Markov regime-switching (Hamilton filter, Kim smoother, EM).** New
  ...
  **EM sanity** — log-likelihood monotone non-decreasing (asserted); label-switching resolved
  ...
  clean. Delivered on branch `feat/timeseries-regime` as **PR #13 — merged `b1d2348`** (main synced,
  branch deleted).

- **Step 22 — Pillar V step 3: multivariate time series (VECM + DCC-GARCH), econometrics arc closed.**
  New `quantica/timeseries/vecm.py` + `dcc.py`, and `simulate_vecm`/`simulate_dcc` in `data.py`.
  **Scope discipline held**: the estimation logic is hand-implemented; statsmodels/numpy are the
  plumbing/anchor. **VECM** — the multivariate generalisation of the stat-arb pillar's pairwise
  cointegration: rank via the existing `johansen` (`select_cointegration_rank`), then a hand-written
  Johansen **reduced-rank regression** (concentrate out lagged diffs → moment matrices → eigenproblem
  → Phillips-normalised β, α = S01 β (β'S11β)⁻¹, short-run Γ + constant by OLS). **DCC-GARCH** —
  univariate GARCH per asset (reusing `fit_volatility_model`) → standardised residuals → Engle's
  dynamic-correlation recursion `Q_t=(1−a−b)Q̄+a z z'+b Q_{t-1}` with (a,b) by quasi-MLE (`scipy`
  Nelder–Mead), giving a time-varying `H_t=D_t R_t D_t` and a one-step-ahead covariance forecast.
  **The cross-pillar tie-back**: `DccCovariance` wraps that forecast as a factor-pillar
  `CovarianceEstimator`, so DCC feeds the *existing* stage-2 OOS comparison harness / min-variance
  machinery directly. Validated (`tests/timeseries/`, +11): **known-truth VECM recovery** (rank,
  β, α, Π), **α/β vs statsmodels VECM to 1e-8**, **n=2 reduces to the Engle–Granger hedge ratio**;
  **known-truth DCC** (a,b + correlation-path tracking, path-corr ~1.0), **positive-definiteness of
  every H_t asserted**, **CCC reduction** (a=b=0 ⇒ constant correlation), and the **tie-back runs
  through `compare_estimators`**. Report `scripts/timeseries_multivariate_report.py` (no network —
  committed FF sample): VECM recovery, DCC correlation, and the DCC-vs-static OOS table → embedded in
  README. **Honest finding reported straight**: on the monthly FF universe DCC does *not* beat
...
- **Step 23 — fEGarch clean-room port, Phase 0: conditional distributions + QMLE engine.** New
  ...
  **each skew reduces exactly to its symmetric base at ξ=1** (known-truth anchor) and genuinely skews
  ...
  `feat/fegarch-phase0`; the fixture reconciliation followed (steps 24–25) and the whole branch
  **merged to `main` as PR #15 (`89ff4e2`)**.

- **Step 24 — fEGarch output-fixture pipeline stood up (on the `feat/fegarch-phase0` branch).**
  `tests/fixtures/fegarch/generate_fixtures.R` — a deterministic R script that **runs fEGarch's
  PUBLIC functions and records their numeric OUTPUT only** (clean-room boundary held: no fEGarch
  source read/opened; the script calls public samplers/fitters/accessors). Uses **fEGarch 1.0.6 on
  R 4.6.1** (both recorded in the manifest); R is **NOT** added to CI — fixtures are generated once
  and committed, and the Python suite stays R-free. **Generated + committed fixtures:**
  (a) **distribution fixtures** — `distribution_moments.csv` + `distribution_quantiles.csv`: for 13
  parameter sets across the eight distributions (`norm`; `std` df∈{5,8}; `ged` shape∈{1,1.5,2};
  `ald` P∈{2,8}; and skewed `snorm`/`sstd`/`sged`/`sald`), 1,000,000 seeded samples each, recording
  empirical mean/variance/skewness/kurtosis and quantiles on a fixed 13-point probability grid.
  (b) **model-fit fixtures** — a committed **synthetic** GARCH(1,1) series `synthetic_returns.csv`
  (n=2500, seeded; **NOT** fEGarch's SP500 data, to avoid redistributing its GPL-3 dataset) fitted
  with fEGarch `garch(1,1)` and `egarch(1,1)` under `norm`, exporting params + log-likelihood + AIC/BIC
  (`fit_*_params.json`) and the conditional-SD series (`fit_*_sigma.csv`). `manifest.json` documents
  every fixture's exact settings/seeds/provenance and lists the pending later-phase fixtures.
  **Immediate findings the fixtures already give (before any Python wiring):** fEGarch's `ged`
  shape=2 is normal (kurtosis ≈ 3, matching our GED convention ✓); all eight distributions are
  standardized to mean 0 / variance 1 ✓; **fEGarch's `ald` at P=8 is near-normal (kurtosis ≈ 3.3),
  so our standardized-Laplace ALD default is wrong** (RECONCILE, needs the paper); and the fEGarch
  GARCH fit recovers the synthetic DGP (α≈0.083 vs 0.08, β≈0.897 vs 0.90). **Python gate unaffected
  (no Python changed): 54 passed / 11 skipped in the fEGarch tests, ruff + interrogate(100%) clean.**
  Next: wire these fixtures into the Python tests (fill the skipped `test_*_matches_fegarch_fixture`
  stubs within Monte-Carlo / exact tolerances) and resolve the ALD + Fernández–Steel RECONCILE flags.

- **Step 25 — fEGarch Phase 0 reconciliation + merge (PR #15 → `main` `89ff4e2`).** Locked the two
  RECONCILE flags against the committed fixtures, clean-room from the equations only (WP 2026-04
  App. C.1 + Fernández–Steel 1998; **no fEGarch source read**). **ALD rewritten** to the **scaled
  average-Laplace (Sargan)** density (Eqs. 31–33, 37): `AverageLaplace(P)` with `P` a fixed integer
  construction parameter (not estimated — fEGarch profiles `P` over a grid, Eq. 59 — so
  `param_names = ()`); logpdf via logsumexp, cdf via the regularized lower incomplete gamma, ppf by
  vectorized bisection, `abs_moment` a(K) via Eq. 37 — exact identities `a(0)=a(2)=1`,
  `a(4)=3+3/(P+1)` (P∈{1,2,8} → 4.5/4.0/3.3̄), matching fEGarch's `ald_P2`/`ald_P8` empirical kurtosis
  (decisively **not** the Laplace's 6). **Fernández–Steel verified, not rewritten** — the branch's
  `μ=M₁(ξ−1/ξ)`, `σ²=(1−M₁²)(ξ²+ξ⁻²)+2M₁²−1` equal App. C.1 Eqs. 39–40 (=Lambert–Laurent); `skew=ξ`
  directly, confirmed against the fixtures. **Wired the fixture-agreement tests** (removed the skips;
  `test_density_matches_fegarch_fixture` over the 13 fixture labels reading P/df/shape/skew from the
  fixture args; `test_ald_form_*`, `test_fernandez_steel_constants_*`) + added ALD moment-identity and
  FS ξ=1-neutral unit tests. **Validation is honest, not masked:** the distribution fixtures are
  1e6-draw MC estimates (fEGarch has no public d/p/q functions), realized quantile deviation ≤0.0153
  (atol 0.02), and **every asserted deviation is ≤2.8 sampling-SE** — genuine Monte-Carlo noise, not a
  systematic disagreement (verified by a z-score audit). `docs/fegarch-spec-notes.md` updated (ALD
  Sargan section; both flags RESOLVED); `.gitignore` excludes the `literature/` reference PDFs.
  Gate green on merged `main`: **1156 passed / 1 skipped** (the lone skip —
  `test_qmle_matches_fegarch_fixture` — is the Phase-1 hook), ruff + mypy + interrogate(100%) clean.
  **Phase 0 COMPLETE; Phase 1 (short-memory models) is the next build step.**

- **Step 26 — fEGarch Phase 1, part 1: GARCH(1,1) (branch `feat/fegarch-phase1`, PR #16, awaiting
  review — not merged).** Clean-room from the equations only (Bollerslev 1986 recursion; WP 2026-04
  App. C.3 conditioning; **no fEGarch source read**). New `quantica/timeseries/fegarch/garch.py`:
  `garch_recursion` (the σ²ₜ = ω + α·ε²ₜ₋₁ + β·σ²ₜ₋₁ variance path, a QMLE `VarianceRecursion`),
  `fit_garch` (constant-mean QMLE under any of the eight distributions, fit on internally rescaled
  returns — the MLE is scale-equivariant — for numerical conditioning of the small ω), `GarchFit`
  result, and `garch_sim` (seeded simulation with burn-in through the Phase-0 distribution layer).
  The engine was **extended, not rebuilt** — a backward-compatible `mean: bool = False` flag on
  `quasi_max_likelihood` puts a constant μ first among the variance params and standardizes
  `(rₜ−μ)/σₜ`. **Pre-sample convention nailed empirically against the σ-fixture:** `σ²₀ = ε²₀ =
  Var(r)` (unbiased, `ddof=1`) reproduces fEGarch's conditional-SD series to **~1e-17**; the biased
  `mean(ε²)` (~1e-4 rel) and unconditional `ω/(1−α−β)` (~1e-2 rel) do not; `presample=50` does not
  change the output. **Headline fixture match** (`test_garch11_norm_matches_fegarch_fixture`, live):
  `fit_garch(synthetic_returns, "norm")` reproduces fEGarch's fit — **params ≤3.4e-5 relative**
  (mu 8.2e-6, omega 3.4e-5, alpha 1.7e-5, beta 2.3e-6), **loglik 6.7e-9**, **AIC/BIC to 1e-7**, and
  the **full conditional-SD series to max 7.6e-6 relative** (these are exact fit fixtures — machine
  order, unlike the MC distribution fixtures). Plus known-truth recovery (planted α,β within a few
  SE), simulation invariants across all eight distributions, and an `arch` cross-check (persistence
  within 0.02, skipped where the local `numba`/`llvmlite` backend is app-control-blocked; CI runs
  it). The generic `test_qmle_matches_fegarch_fixture` skip is retired — the fit-match is now split
  per model in `test_garch.py`; `test_egarch11_matches_fegarch_fixture` stays skipped for Phase 2.
...
(VECM + DCC) are all done. The active build is the **`fEGarch` clean-room port** (sub-project within
Pillar V) — **Phase 0 (conditional distributions + QMLE engine) is COMPLETE and merged (PR #15,
`89ff4e2`)**, fixture-validated against fEGarch; **Phase 1 (short-memory models: GARCH / GJR / TGARCH
/ APARCH through the unified QMLE interface) is the next build step.** Full state above and in
[`docs/fegarch-port-roadmap.md`](docs/fegarch-port-roadmap.md); the port proceeds solo under the four
binding clean-room constraints in CLAUDE.md §12. Other optional depth (none blocking): BEKK/GO-GARCH
...
**Tool-gap logged (step 22) — no reference library for DCC, and the VECM anchor is fixed-config.**
For **VECM**, statsmodels' `VECM` is a clean anchor: at the same `(rank, k_ar_diff, deterministic)`
its α/β match our hand-written reduced-rank estimator to ~1e-8 (same estimator, same normalisation),
so the coefficients are pinned — but the *rank-selection* decision and the deterministic-term
conventions differ across libraries, so the anchor is per-configuration, not turnkey. For **DCC**,
there is no standard Python reference implementation to benchmark against (the `arch` package fits
the univariate GARCH but ships no multivariate DCC), so the estimator is validated by **known-truth
recovery** (a simulated DCC with a known correlation path), the **CCC reduction** (a=b=0), and
**positive-definiteness at every step**, rather than against an external DCC. Two honest corollaries,
surfaced not hidden: (i) the two-stage GARCH pre-filter makes the DCC persistence `b` noisy in finite
...
**Tool-gap logged (step 24) — fEGarch exposes no public density/CDF/quantile functions, only
samplers, so the distribution fixtures are statistical, not exact-point.** fEGarch's public API has
the standardized *samplers* `r{norm,std,ged,ald,snorm,sstd,sged,sald}_s` but **no** public
`d*`/`p*`/`q*` density/CDF/quantile functions (`find_dist`/`distr_est` are *estimators* that take
data, not density-function factories). Under the clean-room rule we may only call public functions,
so the distribution fixtures capture **sampler output** — empirical moments + quantiles from 1e6
seeded draws — and the Python tests will match analytic values within **Monte-Carlo tolerance**
(~1e-2 on central quantiles) rather than to machine precision. The **model-fit** fixtures (params /
log-likelihood / conditional-SD series from `garch()`/`fEGarch()`) are exact, so the QMLE/model
validation stays tight; only the distribution-layer agreement is MC-bounded. (Reading a public
density function, were one exported, would have been permitted — evaluating a documented public
function is capturing output, not reading source.)
...
spacing in embedded examples); unpinned, CI (0.16.0) failed `ruff format --check` on
*pre-existing* docstrings while local (0.15.x) passed. Pinning kept the format gate
reproducible and avoided polluting the rates PR with an unrelated repo-wide reformat. **The
resolution is to unpin ruff and land the reformat as its own standalone `style:` commit when
convenient** — recorded here so the pin doesn't age silently.
...
**~Fifteen blog posts are drafted or obvious, awaiting number-verification (re-run each source
...
14. One pillar's output is another's input — the coherence payoff. VECM generalises the stat-arb
    pillar's pairwise cointegration (matching statsmodels to 1e-8; reducing to the exact pairwise
    hedge ratio for n=2), and DCC-GARCH produces the *same* conditional-covariance object the
    portfolio min-variance and risk VaR machinery consume — so it drops into the factor-stage-2 OOS
    harness as a drop-in estimator. The honest verdict: DCC doesn't beat Ledoit–Wolf shrinkage on
    the monthly universe (estimation noise at small n/T), which *is* the estimation-error thesis —
    the value is the validated coherence across pillars, not a horse-race win.
15. *(drafted, topic TBD-in-notes)* — plus the flagship narrative post tying the
